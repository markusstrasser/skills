...
- **[2026-06-18] FIXED — scout timeout cascaded into all axes skipped as budget_exhausted.** Premise scout shared the axis `DispatchBudget` clock; a 90s scout timeout left insufficient remaining time for 600s-profile axes, so dispatch produced nothing while mislabeling the skip. Scout now runs outside the axis budget; the budget clock restarts after scout. `budget_insufficient_for_profile` distinguishes a cap smaller than profile timeout from mid-run exhaustion.
