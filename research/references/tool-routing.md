...
| ~~`mcp__perplexity__perplexity_search`~~ | **SUPERSEDED — use Exa.** Raw web search results (Search API) | Cost-tied with Exa `/answer` (~$0.005/call) but lower precision (75% vs 92% at N=60) and no synthesis edge. No remaining use case — route URL discovery / fact-checking to `web_search_exa` or `verify_claim`. |
| `mcp__perplexity__perplexity_ask` | Grounded factual answer (Sonar Pro) | **Biomedical precise-number claims ONLY.** Empirically 5-0 vs Exa `/answer` on genomics precision (Mar 2026 N=5) — this is its sole retained niche. Elsewhere it's the only engine below the random-TRUE baseline (75% vs Exa 92% at N=60 2026-05-19; 30% on scholar-attribution). Default to Exa/`verify_claim` for everything non-biomedical. Cost ~$0.01-0.05/call. Batch safe at N≤10, risky at N≥25 (rubber-stamps user-supplied specifics). |
...
5. **Perplexity `ask` is biomedical-precise-number-only; `search` is retired.** `perplexity_search` is superseded by Exa (cost-tied, lower precision). `perplexity_ask` retains exactly one niche — genomics/biomedical precise-number claims (5-0 vs Exa, N=5). Everywhere else it's at or below the random-TRUE baseline (75% N=60; 30% on context-scholar). Default non-biomedical factual lookups to Exa/`verify_claim`.
...
## fetch_paper failure recovery — green-OA / Cloudflare retrieval ladder

> Moved from research/SKILL.md (2026-07-06, progressive disclosure). Rare-failure recovery
> path when Sci-Hub + Unpaywall both miss; SKILL.md keeps a one-line pointer.
