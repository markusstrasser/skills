...
description: Versioned dossiers for entities (people, companies, genes, drugs, stocks). One file per entity, git-versioned, every claim sourced. Use for profiling, tracking, structured notes.
user-invocable: true
...
## Onboarding Evidence Bundle (intel project — public companies)

When building an entity file for a US-listed public company, **always check `.scratch/onboarding/<TICKER>/` first**. `tools/onboard_ticker.py` auto-generates three evidence files at onboarding time:

- `dilution.md` — multi-year EDGAR aggregator (424B5 + 8-K Items 1.01/1.02/2.03/3.02). Surfaces convertible-refi rolling patterns. Generated by `tools/dilution_history.py`.
- `litigation.md` — CourtListener `caseName:` search. Surfaces securities class actions, fraud cases. Generated by `tools/entity_litigation_history.py`.
- `auditor.md` — iXBRL `dei:AuditorName` from latest 10-K. Flags mid-tier auditors. Generated by `tools/extract_auditor.py`.

**Required entity-file sections** for any non-trivial public-company workup (BUY / HOLD / AVOID convictions):

- `## Historical Dilution Pattern` — embed `dilution.md` content with provenance tags
- `## Litigation History` — embed `litigation.md`; flag securities/fraud-relevant clusters
- `## Auditor` — embed `auditor.md`; flag mid-tier or unusual

If the bundle is missing (rare — onboarding ran without `--no-evidence`), regenerate with:
```bash
uv run python3 tools/onboard_ticker.py TICKER  # idempotent; refreshes evidence
```

**Why this is mandatory:** the AAOI miss 2026-05-07 (entity file caught current-cycle dilution but missed the 9-year refi treadmill + 2017-2019 securities class action + Grant Thornton mid-tier auditor) showed that without these three sections, our entity files surface the structural-mechanism bear case but miss the fraud-pattern bear case. Both are real; both are required. See `analysis/postmortems/2026-05-07_hims_avoid_postmortem.md` for context.
