...
The precise values, curves, and rules behind the review. Cite these in findings instead of approximating. Distilled from Emil Kowalski's design engineering philosophy (animations.dev; source: emilkowalski/skills, MIT), adapted for a Svelte/plain-CSS stack.
...
Find curves at [easing.dev](https://easing.dev/) or [easings.co](https://easings.co/) — don't hand-roll from scratch. In Svelte, `svelte/easing`'s `cubicOut`/`quintOut` are the JS-side equivalents; pass them explicitly — `svelte/transition` defaults are weak.
...
- **Origin-aware popovers.** Scale from the trigger, not center. Set `transform-origin` to the trigger side (or the positioning library's CSS variable if one exists — Radix exposes `--radix-popover-content-transform-origin`, Base UI `--transform-origin`). **Modals are exempt** — they appear centered in the viewport, keep `transform-origin: center`.
...
- Apple-style config (easier to reason about): duration + bounce, e.g. `{ duration: 0.5, bounce: 0.2 }` in Motion.
- Traditional physics (more control): mass / stiffness / damping. Svelte: `Spring` from `svelte/motion` with `{ stiffness, damping }`.
...
Pointer-tracking interactions: interpolate through a spring rather than tying the value directly to pointer position (direct = artificial, no momentum). Only when the motion is decorative — a functional value readout should not animate.
...
Legacy fallback: flip a `data-mounted` attribute after mount and transition on it.
...
- **Don't drive child transforms via a CSS variable on the parent** — it recalcs styles for all children. Set `transform` directly on the element.
  ...
  ```
- **CSS animations beat main-thread JS under load** — CSS/WAAPI run off the main thread; rAF-driven animation (most JS animation libraries' default path, including Svelte's `Spring`/`Tween` and Motion's `x`/`y` shorthands) stutters while the browser loads/scripts/paints. Use CSS for predetermined motion, JS for dynamic/interruptible.
