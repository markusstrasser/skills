...
brainstorm/references/llmx-dispatch.md | 37 ++++++++-------
 hooks/generate-overview.sh             | 74 +++++++++++++++++++++--------
 hooks/pretool-llmx-guard.sh            | 10 ++--
 improve/SKILL.md                       | 27 ++++++-----
 llmx-guide/SKILL.md                    | 29 +++++++++---
 observe/SKILL.md                       | 50 +++++++------------
 research-ops/SKILL.md                  | 26 +++++-----
 research-ops/scripts/run-cycle.sh      | 49 +++++++++++++------
 review/SKILL.md                        | 10 +++-
 review/scripts/model-review.py         | 87 +++++++++++-----------------------
 review/scripts/test_model_review.py    | 19 ++++++--
 11 files changed, 234 insertions(+), 184 deletions(-)
```
...
hooks/generate-overview.sh --- 4/6 --- Bash
 ...
 ..                                      121 }
 97                                      122 
 98 # --- Generate a single overview --  123 # --- Generate a single overview --
 .. -                                    ... -
 99 generate_one() {                     124 generate_one() {

hooks/generate-overview.sh --- 5/6 --- Bash
138 163 
139 164   mkdir -p "$output_dir"
140 165 
141 166   local temp_prompt dispatch_profile
142 167   temp_prompt=$(mktemp /tmp/overview-prompt-$$-${type}-XXXXXX.txt)
... 168   dispatch_profile=$(resolve_overview_profile) || return 1
143 169 
144 170   # Step 1: Extract content with repomix (--stdout avoids clipboard races)
145 171   local include_pattern=""

hooks/generate-overview.sh --- 6/6 --- Bash
178   prompt_size=$(wc -c < "$temp_prom  204   prompt_size=$(wc -c < "$temp_prom
... pt")                                 ... pt")
179   prompt_tokens=$((prompt_size / 4)  205   prompt_tokens=$((prompt_size / 4)
... )                                    ... )
180                                      206 
181   echo "[$type] Generating (~${prom  207   echo "[$type] Generating (~${prom
... pt_tokens} tokens, model: $OVERVIEW  ... pt_tokens} tokens, profile: $dispat
... _MODEL)..."                          ... ch_profile)..."
182                                      208 
183   # Step 4: Check token estimate ag  209   # Step 4: Check token estimate ag
... ainst model limits                   ... ainst model limits
184   if [[ $prompt_tokens -gt 900000 ]  210   if [[ $prompt_tokens -gt 900000 ]
... ]; then                              ... ]; then
185     echo "[$type] ERROR: prompt (~$  211     echo "[$type] ERROR: prompt (~$
... {prompt_tokens} tokens) exceeds saf  ... {prompt_tokens} tokens) exceeds saf
... e limit for $OVERVIEW_MODEL. Tighte  ... e limit for $dispatch_profile. Tigh
... n OVERVIEW_EXCLUDE or dirs." >&2     ... ten OVERVIEW_EXCLUDE or dirs." >&2
186     rm -f "$temp_prompt"             212     rm -f "$temp_prompt"
187     return 1                         213     return 1
188   fi                                 214   fi
189                                      215 
190   # Step 5: Generate via llmx (atom  216   # Step 5: Generate via shared dis
... ic write — temp file, mv on success  ... patch (atomic write — temp file, mv
... )                                    ...  on success)
191   local llmx_stderr llmx_output      217   local dispatch_meta dispatch_erro
...                                      ... r llm_output dispatch_exit resolved
...                                      ... _model
192   llmx_stderr=$(mktemp /tmp/overvie  218   dispatch_meta=$(mktemp /tmp/overv
... w-llmx-stderr-XXXXXX)                ... iew-dispatch-meta-XXXXXX.json)
...                                      219   dispatch_error=$(mktemp /tmp/over
...                                      ... view-dispatch-error-XXXXXX.json)
193   llmx_output=$(mktemp "${output_di  220   llm_output=$(mktemp "${output_dir
... r}/.overview-tmp-${type}-XXXXXX")    ... }/.overview-tmp-${type}-XXXXXX")
194                                      221 
195   # Disable errexit to capture exit  222   # Disable errexit to capture exit
...  code (set -e would skip cleanup on  ...  code (set -e would skip cleanup on
...  failure)                            ...  failure)
196   set +e                             223   set +e
...                                      224   uv run python3 "$SCRIPT_DIR/../sc
...                                      ... ripts/llm-dispatch.py" \
...                                      225     --profile "$dispatch_profile" \
197   cat "$temp_prompt" | timeout 300   226     --context "$temp_prompt" \
... llmx chat -m "$OVERVIEW_MODEL" 2>"$  ... 
... llmx_stderr" > "$llmx_output"        ... 
...                                      227     --prompt "Write the requested c
...                                      ... odebase overview in markdown." \
...                                      228     --output "$llm_output" \
...                                      229     --meta "$dispatch_meta" \
...                                      230     --error-output "$dispatch_error
...                                      ... " \
...                                      231     >/dev/null
198   local llmx_exit=$?                 232   dispatch_exit=$?
199   set -e                             233   set -e
200                                      234 
201   # Cleanup prompt (no longer neede  235   # Cleanup prompt (no longer neede
... d)                                   ... d)
202   rm -f "$temp_prompt"               236   rm -f "$temp_prompt"
203                                      237 
204   # Check for failure: non-zero exi  238   # Check for failure: non-zero exi
... t or empty output                    ... t or empty output
205   if [[ $llmx_exit -ne 0 ]] || [[ !  239   if [[ $dispatch_exit -ne 0 ]] || 
...  -s "$llmx_output" ]]; then          ... [[ ! -s "$llm_output" ]]; then
206     echo "[$type] ERROR: llmx faile  240     echo "[$type] ERROR: dispatch f
... d (exit=$llmx_exit). stderr:" >&2    ... ailed (exit=$dispatch_exit)." >&2
207     cat "$llmx_stderr" >&2           241     [[ -f "$dispatch_error" ]] && c
...                                      ... at "$dispatch_error" >&2
...                                      242     [[ -f "$dispatch_meta" ]] && ca
...                                      ... t "$dispatch_meta" >&2
208     rm -f "$llmx_stderr" "$llmx_out  243     rm -f "$dispatch_error" "$dispa
... put"                                 ... tch_meta" "$llm_output"
209     return 1                         244     return 1
210   fi                                 245   fi
...                                      246   resolved_model=$(python3 -c 'impo
...                                      ... rt json,sys; print(json.load(open(s
...                                      ... ys.argv[1])).get("resolved_model","
...                                      ... unknown"))' "$dispatch_meta" 2>/dev
...                                      ... /null || echo "unknown")
211   rm -f "$llmx_stderr"               247   rm -f "$dispatch_error"
212                                      248 
213   # Step 6: Prepend freshness metad  249   # Step 6: Prepend freshness metad
... ata to temp output, then atomic mv   ... ata to temp output, then atomic mv
214   local git_sha gen_ts meta_line     250   local git_sha gen_ts meta_line
215   git_sha=$(echo "$COMMIT_HASH" | h  251   git_sha=$(echo "$COMMIT_HASH" | h
... ead -c 7)                            ... ead -c 7)
216   gen_ts=$(date -u +"%Y-%m-%dT%H:%M  252   gen_ts=$(date -u +"%Y-%m-%dT%H:%M
... :%SZ")                               ... :%SZ")
217   meta_line="<!-- Generated: ${gen_  253   meta_line="<!-- Generated: ${gen_
... ts} | git: ${git_sha} | model: ${OV  ... ts} | git: ${git_sha} | profile: ${
... ERVIEW_MODEL} -->"                   ... dispatch_profile} | model: ${resolv
...                                      ... ed_model} -->"
218                                      254 
219   local tmp_final                    255   local tmp_final
220   tmp_final=$(mktemp "${output_dir}  256   tmp_final=$(mktemp "${output_dir}
... /.overview-final-${type}-XXXXXX")    ... /.overview-final-${type}-XXXXXX")
221   { echo "$meta_line"; echo ""; cat  257   { echo "$meta_line"; echo ""; cat
...  "$llmx_output"; } > "$tmp_final"    ...  "$llm_output"; } > "$tmp_final"
222   rm -f "$llmx_output"               258   rm -f "$llm_output" "$dispatch_me
...                                      ... ta"
223                                      259 
224   # Atomic move — old overview pres  260   # Atomic move — old overview pres
... erved until this succeeds            ... erved until this succeeds
225   mv "$tmp_final" "$output_file"     261   mv "$tmp_final" "$output_file"

hooks/pretool-llmx-guard.sh --- Bash
 98 # The CLI has 5 known failure modes   98 # The CLI has 5 known failure modes
 ..  in agent context (multi-file drops   ..  in agent context (multi-file drops
 .. , 0-byte -o,                          .. , 0-byte -o,
 99 # rate limit loops, stdin EOF, pipe   99 # rate limit loops, stdin EOF, pipe
 ..  masking). The Python API bypasses    ..  masking). The Python API bypasses 
 .. all of them.                          .. all of them.
100 if echo "$CMD" | grep -qE 'llmx\s+c  100 if echo "$CMD" | grep -qE 'llmx\s+c
... hat'; then                           ... hat'; then
101   echo "[llmx-guard] BLOCKED: Do no  101   echo "[llmx-guard] BLOCKED: Do no
... t use 'llmx chat' CLI. Use the Pyth  ... t use 'llmx chat' CLI for automatio
... on API instead:" >&2                 ... n. Use the shared dispatch helper i
...                                      ... nstead:" >&2
...                                      102   echo "  uv run python3 ~/Projects
...                                      ... /skills/scripts/llm-dispatch.py \\"
...                                      ...  >&2
...                                      103   echo "    --profile fast_extract 
...                                      ... \\" >&2
102   echo "  from llmx.api import chat  104   echo "    --context context.md \\
...  as llmx_chat" >&2                   ... " >&2
103   echo "  r = llmx_chat(prompt=...,  105   echo "    --prompt 'Analyze this'
...  provider='google', model='gemini-3  ...  \\" >&2
... .1-pro-preview', api_only=True, tim  ... 
... eout=300)" >&2                       ... 
104   echo "  Bootstrap: sys.path.inser  106   echo "    --output /tmp/result.md
... t(0, glob.glob(str(Path.home() / '.  ... " >&2
... local/share/uv/tools/llmx/lib/pytho  ... 
... n*/site-packages'))[0])" >&2         ... 
105   ~/Projects/skills/hooks/hook-trig  107   ~/Projects/skills/hooks/hook-trig
... ger-log.sh "llmx-chat-blocked" "blo  ... ger-log.sh "llmx-chat-blocked" "blo
... ck" "$(echo "$CMD" | head -c 200)"   ... ck" "$(echo "$CMD" | head -c 200)" 
... 2>/dev/null || true                  ... 2>/dev/null || true
106   exit 2                             108   exit 2
107 fi                                   109 fi
...
... hat                                  ... 
...                                      186 from shared.llm_dispatch import dis
...                                      ... patch
186                                      187 
187 transcripts = Path("$ARTIFACT_DIR/i  188 transcripts = Path("$ARTIFACT_DIR/i
... nput.md").read_text()                ... nput.md").read_text()
188 response = llmx_chat(                189 response = dispatch(
189     prompt=transcripts + "\n\nAnaly  190     profile="deep_review",
... ze Claude Code session transcripts   ... 
... for repeated workflows. "            ... 
...                                      191     prompt=(
...                                      192         "Analyze Claude Code sessio
...                                      ... n transcripts for repeated workflow
...                                      ... s. "
190     "Classify as SKILL candidate (m  193         "Classify as SKILL candidat
...
... able). "                             ... reusable). "
191     "For each: pattern, frequency,   194         "For each: pattern, frequen
... current cost, trigger, parameters,   ... cy, current cost, trigger, paramete
... skeleton. "                          ... rs, skeleton. "
192     "Only patterns appearing 2+ tim  195         "Only patterns appearing 2+
... es across different sessions. Max 7  ...  times across different sessions. M
...  candidates. "                       ... ax 7 candidates. "
193     "Rank by frequency x complexity  196         "Rank by frequency x comple
...  saved.",                            ... xity saved."
194     provider="google",               197     ),
195     model="gemini-3.1-pro-preview",  198     context_text=transcripts,
196     timeout=300,                     199     output_path=Path("$ARTIFACT_DIR
...                                      ... /candidates.md"),
197 )                                    200 )
198 ```                                  201 ```
199                                      202 
...
270 ```bash                              273 ```bash
271 CLAUDE_PROCS=$(pgrep -lf claude 2>/  274 CLAUDE_PROCS=$(pgrep -lf claude 2>/
... dev/null | wc -l | tr -d ' ')        ... dev/null | wc -l | tr -d ' ')
272 ```                                  275 ```
273 If >= 5: skip subagent dispatch (Ti  276 If >= 5: skip subagent dispatch (Ti
...
...                                      ... le cheap_tick ...`.
274                                      277 
275 ### Each Tick: Pick ONE Task by Pri  278 ### Each Tick: Pick ONE Task by Pri
... ority                                ... ority
276                                      279 
...
llmx-guide/SKILL.md --- 1/4 --- Text
 ...
 ..                                       25 
 11 > Detail files in `references/`: [m   26 > Detail files in `references/`: [m
 ...
 12                                       27 
 13 ## Before You Call llmx — Checklist   28 ## Before You Call llmx — Checklist
 14                                       29 
 15 1. **Model name correct?** Hyphens    30 1. **Model name correct?** Hyphens 
 .. not dots (`claude-sonnet-4-6` not `   .. not dots (`claude-sonnet-4-6` not `
 .. claude-sonnet-4.6`)                   .. claude-sonnet-4.6`)
 16 2. **Timeout set?** Reasoning model   31 2. **Timeout set?** Reasoning model
 ...
 ..                                       ..  force `-p codex-cli`.
 21 7. **Hangs in agent context?** Clau   36 7. **Hangs in agent context?** Clau
 ...
 22 8. **Prompt is positional, context    37 8. **Prompt is positional, context 
 ...
 23 9. **For critical reviews, use one    38 9. **For critical reviews, use one 
 ...
 ..                                       .. e.
 24                                       39 
 25 ## When llmx Fails — Diagnose, Don'   40 ## When llmx Fails — Diagnose, Don'
 .. t Downgrade                           .. t Downgrade
 26                                       41 

llmx-guide/SKILL.md --- 2/4 --- Text
 56 If the task is high-stakes or revie   71 If the task is high-stakes or revie
 .. w-oriented, do this:                  .. w-oriented, do this:
 57                                       72 
 58 ```bash                               73 ```bash
 59 cat overview.md diff.md touched-fil   74 awk 'FNR==1{print "\n# File: " FILE
 .. es.md > combined-context.md           .. NAME "\n"}1' overview.md diff.md to
 ..                                       .. uched-files.md > combined-context.m
 ..                                       .. d
 60 llmx chat -m gemini-3.1-pro-preview   75 llmx chat -m gemini-3.1-pro-preview
...
llmx-guide/SKILL.md --- 3/4 --- Text
 72                                       87 
 73 ### 2. GPT-5.x Timeouts               88 ### 2. GPT-5.x Timeouts
 74                                       89 
 75 GPT-5.4 with reasoning burns time B   90 GPT-5.4 with reasoning burns time B
 ...
 76                                       91 
 77 **`max_completion_tokens` includes    92 **`max_completion_tokens` includes 
...
123 set -o pipefail                      138 set -o pipefail
124 llmx chat -m gpt-5.4 --debug -o /tm  139 llmx chat -m gpt-5.4 --debug -o /tm
... p/review.md "query" 2> /tmp/review.  ... p/review.md "query" 2> /tmp/review.
... err                                  ... err
125 echo $?                              140 echo $?
126 sed -n '1,80p' /tmp/review.err       141 tail -n 200 /tmp/review.err
127 sed -n '1,80p' /tmp/review.md        142 sed -n '1,80p' /tmp/review.md
128 ```                                  143 ```
129                                      144 
130 From Claude Code: set Bash tool `ti  145 From Claude Code: set Bash tool `ti
... meout: 660000` (11 min) — must exce  ... meout: 1200000` (20 min) — it must 
... ed llmx's `--timeout`.               ... exceed llmx's `--timeout`.
131                                      146 
132 ### 4. shell=True + Parentheses      147 ### 4. shell=True + Parentheses
133                                      148 

observe/SKILL.md --- 1/3 --- Text
 97                                       97 
 98 Send full-fidelity transcript + cov   98 Send full-fidelity transcript + cov
 .. erage digest + operational context    .. erage digest + operational context 
 .. to Gemini 3.1 Pro. Full prompt in `   .. to Gemini 3.1 Pro. Full prompt in `
 .. references/gemini-dispatch-prompt.m   .. references/gemini-dispatch-prompt.m
 .. d`.                                   .. d`.
 99                                       99 
100 Dispatch via llmx Python API (not C  100 Dispatch via the shared wrapper, no
... LI subprocess). Write the dispatch   ... t raw `llmx` calls:
... as a short inline script:            ... 
101                                      101 
102 ```bash                              102 ```bash
103 uv run python3 << 'PYEOF'            103 cat "$HOME/Projects/meta/artifacts/
...                                      ... observe/input.md" \
104 import sys, glob                     ... 
105 from pathlib import Path             ... 
106                                      ... 
107 # Bootstrap llmx from uv tool venv   ... 
108 _site = glob.glob(str(Path.home() /  ... 
...  ".local/share/uv/tools/llmx/lib/py  ... 
... thon*/site-packages"))               ... 
109 if _site: sys.path.insert(0, _site[  ... 
... 0])                                  ... 
110 sys.path.insert(0, str(Path.home()   104   "$HOME/Projects/meta/artifacts/ob
... / "Projects/llmx"))                  ... serve/coverage-digest.txt" \
111 from llmx.api import chat as llmx_c  105   > /tmp/observe-context.md
... hat                                  ... 
112                                      ... 
113 artifact_dir = Path("$HOME/Projects  106 uv run python3 ~/Projects/skills/sc
... /meta/artifacts/observe").expanduse  ... ripts/llm-dispatch.py \
... r()                                  ... 
114 context = (artifact_dir / "input.md  107   --profile deep_review \
... ").read_text()                       ... 
115 coverage = (artifact_dir / "coverag  108   --context /tmp/observe-context.md
... e-digest.txt").read_text()           ...  \
116 prompt = Path("$CLAUDE_SKILL_DIR/re  109   --prompt-file "$CLAUDE_SKILL_DIR/
... ferences/gemini-dispatch-prompt.md"  ... references/gemini-dispatch-prompt.m
... ).read_text()                        ... d" \
117                                      ... 
118 response = llmx_chat(                ... 
119     prompt=context + "\n\n" + cover  110   --output "$HOME/Projects/meta/art
... age + "\n\n" + prompt,               ... ifacts/observe/gemini-output.md" \
120     provider="google",               111   --meta "$HOME/Projects/meta/artif
...                                      ... acts/observe/gemini-output.meta.jso
...                                      ... n" \
121     model="gemini-3.1-pro-preview",  ... 
122     temperature=1.0,                 ... 
123     timeout=300,                     112   --error-output "$HOME/Projects/me
...                                      ... ta/artifacts/observe/gemini-output.
...                                      ... error.json"
124 )                                    ... 
125 (artifact_dir / "gemini-output.md")  ... 
... .write_text(response.content)        ... 
126 print(f"✓ Gemini output: {len(respo  ... 
... nse.content)} chars, {response.late  ... 
... ncy:.1f}s")                          ... 
127 PYEOF                                ... 
128 ```                                  113 ```
129                                      114 
130 Substitute `$HOME` and `$CLAUDE_SKI  ... 
... LL_DIR` with actual paths before ru  ... 
... nning.                               ... 
131                                      ... 
132 ### Step 3: Stage Findings           115 ### Step 3: Stage Findings
133                                      116 
134 Validate Gemini output against tran  117 Validate Gemini output against tran
... script, check session UUIDs, save a  ... script, check session UUIDs, save a
... s JSON artifact. Full procedure and  ... s JSON artifact. Full procedure and
...  JSON template in `references/findi  ...  JSON template in `references/findi
... ngs-staging.md`.                     ... ngs-staging.md`.

observe/SKILL.md --- 2/3 --- Text
186                                      169 
187 ### Phase 2: Pattern Extraction (Ge  170 ### Phase 2: Pattern Extraction (Ge
... mini)                                ... mini)
188                                      171 
189 Dispatch to Gemini 3.1 Pro for stru  172 Dispatch to Gemini 3.1 Pro for stru
... ctured pattern extraction. Full pro  ... ctured pattern extraction. Full pro
... mpt in `references/gemini-prompt.md  ... mpt in `references/gemini-prompt.md
... `. Use `llmx.api.chat()` (Python AP  ... `. Use the shared dispatch helper, 
... I), not CLI subprocess.              ... not raw CLI subprocess calls.
190   
...
imate
  ...
  if [[ $prompt_tokens -gt 900000 ]]; then
    echo "[$type] ERROR: prompt (~${prompt_tokens} tokens) exceeds safe limit for $dispatch_profile. Tighten OVERVIEW_EXCLUDE or dirs." >&2
    rm -f "$temp_prompt"
    return 1
  fi
...
PROMPT_EOF
    ...
    if [ "$dispatch_exit" -eq 0 ] && [ -s "$cycle_output" ]; then
        ...
        if [ -f "$cycle_error" ]; then
            cat "$cycle_error" >&2
        elif [ -f "$cycle_meta" ]; then
            cat "$cycle_meta" >&2
        fi
    fi
...
**What still forces API:** `--max-tokens` (CLI caps at 8
...
```
Look for patterns -- if 3 commits fix hook edge cases, that s

... [truncated for review packet] ...

 | HTTP-probe URLs, check release pages |
...
(Omitted 3 additional touched files from excerpts.)
