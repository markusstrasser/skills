...
- file_char_cap: `8000`
- max_file_count: `12`
- budget_enforcement: `{'usage': 23413, 'limit': 120000, 'metric': 'tokens', 'estimate_method': 'heuristic:chars_div_4', 'dropped_blocks': [], 'truncated_blocks': []}`
...
- `hooks/postcommit-skill-validate.sh`
- `hooks/postmerge-overview.sh`
- `hooks/pretool-llmx-guard.sh`
- `hooks/prewrite-frontmatter-inject.sh`
- `hooks/sessionend-index-sessions.sh`
...
- `research-ops/scripts/gather-cycle-state.sh`
...
M .claude/overview-marker-source
 ...
 M hooks/postcommit-skill-validate.sh
 M hooks/postmerge-overview.sh
 M hooks/pretool-llmx-guard.sh
 M hooks/prewrite-frontmatter-inject.sh
 M hooks/sessionend-index-sessions.sh
 ...
 M research-ops/scripts/gather-cycle-state.sh
 ...
 M review/scripts/test_build_plan_close_context.py
...
.claude/overview-marker-source                  |   2 +-
 ...
 constitution/SKILL.md                           |   2 +-
 goals/SKILL.md                                  |   4 +-
 hooks/epistemic-domain-router.sh                |   4 +-
 hooks/generate-overview-batch.sh                | 345 +--------------------
 hooks/generate-overview.sh                      | 281 +----------------
 hooks/overview-staleness-cron.sh                |  24 +-
 hooks/permission-auto-allow.sh                  |   2 +-
 hooks/postcommit-skill-validate.sh              |   2 +-
 hooks/postmerge-overview.sh                     |   5 -
 hooks/pretool-llmx-guard.sh                     |  41 ++-
 hooks/prewrite-frontmatter-inject.sh            |   4 +-
 hooks/sessionend-index-sessions.sh              |   2 +-
 hooks/sessionend-overview-trigger.sh            |  13 +-
 improve/SKILL.md                                |  50 +--
 llmx-guide/SKILL.md                             |  42 ++-
 observe/SKILL.md                                |  70 ++---
 observe/scripts/validate_session_ids.py         |   2 +-
 research-ops/SKILL.md                           |  34 ++-
 research-ops/scripts/gather-cycle-state.sh      |   2 +-
 research-ops/scripts/run-cycle.sh               |  52 +++-
 research/SKILL.md                               |   2 +-
 review/SKILL.md                                 |  10 +-
 review/lenses/plan-close-review.md              |   4 +
 review/references/context-assembly.md           |  17 ++
 review/scripts/build_plan_close_context.py      | 335 ++++++++------------
 review/scripts/model-review.py                  | 388 +++++++++++-------------
 review/scripts/test_build_plan_close_context.py |  34 +++
 review/scripts/test_model_review.py             |  71 ++++-
 upgrade/SKILL.md                                |   8 +
 34 files changed, 745 insertions(+), 1267 deletions(-)
```
...
constitution/SKILL.md --- Text
44 - **Phase 0 steering report** — corr  44 - **Phase 0 steering report** — corr
.. ections, feedback, hook signals       .. ections, feedback, hook signals
45 </exploration_checklist>              45 </exploration_checklist>
46                                       46 
47 Also read ~/Projects/meta/ files for  47 Also read ~/Projects/agent-infra/ fi
..  the philosophical framework:         .. les for the philosophical framework:
48 - constitutional-delta.md (the delta  48 - constitutional-delta.md (the delta
..  between Claude's built-in constitut  ..  between Claude's built-in constitut
.. ion and project needs)                .. ion and project needs)
49 - philosophy-of-epistemic-agents.md   49 - philosophy-of-epistemic-agents.md 
.. (epistemic foundations)               .. (epistemic foundations)
50 - frontier-agentic-models.md (what r  50 - frontier-agentic-models.md (what r
.. esearch says about agent reliability  .. esearch says about agent reliability
.. )                                     .. )

goals/SKILL.md --- 1/2 --- Text
 49 - MEMORY.md or persistent memory (p   49 - MEMORY.md or persistent memory (p
 .. rior decisions, constitutional ques   .. rior decisions, constitutional ques
 .. tionnaire results)                    .. tionnaire results)
 50 - Any README, docs/, or project des   50 - Any README, docs/, or project des
 .. cription files                        .. cription files
 51 - Recent git log (what has the huma   51 - Recent git log (what has the huma
 .. n actually been working on vs. what   .. n actually been working on vs. what
 ..  they say they want?)                 ..  they say they want?)
 52 - `$HOME/Projects/meta/memory/MEMOR   52 - `$HOME/Projects/agent-infra/memor
 .. Y.md` (cross-project decisions if i   .. y/MEMORY.md` (cross-project decisio
 .. t exists)                             .. ns if it exists)
 53 - **Phase 0 steering report** (corr   53 - **Phase 0 steering report** (corr
 .. ections, feedback, topic drift, cos   .. ections, feedback, topic drift, cos
 .. t allocation)                         .. t allocation)
 54 </exploration>                        54 </exploration>
 55                                       55 

goals/SKILL.md --- 2/2 --- Text
101                                      101 
102 ## Cross-Project Awareness           102 ## Cross-Project Awareness
103                                      103 
104 If `$HOME/Projects/meta/memory/MEMO  104 If `$HOME/Projects/agent-infra/memo
... RY.md` exists, read it for cross-pr  ... ry/MEMORY.md` exists, read it for c
... oject decisions. Goals in one proje  ... ross-project decisions. Goals in on
... ct may constrain goals in another:   ... e project may constrain goals in an
...                                      ... other:
105 - Shared entity graph means shared   105 - Shared entity graph means shared 
... data investments                     ... data investments
106 - Shared epistemics means compatibl  106 - Shared epistemics means compatibl
... e evidence standards                 ... e evidence standards
107 - Time/attention is zero-sum across  107 - Time/attention is zero-sum across
...  projects                            ...  projects

hooks/epistemic-domain-router.sh --- Bash
23       DOMAIN="trading"                23       DOMAIN="trading"
24       ;;                              24       ;;
...
lls/architect/run.sh decide {run-id} approve {winner-id} "Clear and simple"
```
...
genuinely different approaches. Novelty rises continuously with denial depth (NEOGAUGE, NAACL 2025). This is the primary divergence mechanism. See `references/llmx-dispatch.md` for prompt templates.
...
l questions (identity, scope, autonomy)
...
```text
(Omitted 22 additional touched files from excerpts.)
```
