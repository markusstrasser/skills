...
> **Opus 4.8 (current, 2026-05-28):** Builds on 4.7. The official 4.7→4.8 migration guide confirms **no breaking API changes** — the version-specific behaviors below (adaptive thinking off by default, `temperature`/`top_p`/prefill → 400, `thinking.display` omitted default, fewer-tools-by-default, 2576px vision, 4.7 tokenizer) all carry forward into 4.8 *unchanged*. **New in 4.8:** effort defaults to `high` (recalibrated — `medium` thinks somewhat more, `high` somewhat less, `xhigh` substantially more vs 4.7; re-baseline before tuning); you can update instructions mid-run by inserting a `role: "system"` message immediately after a user turn in `messages` (no prompt-cache break, no user-turn detour; 4.7 and earlier reject this with 400); refusal `stop_details` now documented; prompt-cache minimum dropped to 1,024 tokens; 4.8 is materially more honest (~4× less likely to let its own code flaws pass unremarked, flags uncertainty more readily).
