...
- **Calibration warning (AA, independent):** AA-Omniscience **Index 31 — below Fable 5's 40**, despite Opus 5 leading the Intelligence Index at 61. Accuracy **+7pp** over Opus 4.8 but **hallucination +14pp → 50%** (the card self-reported only "+6%, slightly more"). It leads on intelligence and trails on confident-wrongness — keep provenance tagging and claim verification on for factual work.
...
- **[2026-07-12b] Routing-bug mechanism clue:** subagent Bash shells export `CLAUDE_CODE_SUBAGENT_MODEL=claude-sonnet-4-6` (the retired harness default) while the same agents self-report serving `claude-sonnet-5` — so a coercion layer maps the retired default to sonnet-5 and apparently wins over `model:` pins. Whoever debugs the pin bug: start at that env var's consumer, not at the Agent-tool param plumbing. (arc-agi 41f9b649, live process-table observation.)
- **[2026-07-12c] Concurrent-fable attribution protocol (proven 6× tonight):** N concurrent `llmx -m claude-fable-5` calls make usage-log TIMESTAMP attribution ambiguous. Verified protocol: attribute by CONTENT-CORRESPONDENCE (the response quotes packet-unique strings/numbers — stronger than timestamps) and state which method was used in the provenance line. Dispatch briefs should pre-authorize this fallback.
- **[2026-07-12d] claude-cli fable transport carries AMBIENT PROJECT CONTEXT beyond the packet:** a fable-max response cited a "37% false positives" figure verbatim from a global CLAUDE.md rule-8 example that was NOT in the 10KB packet (arc-agi frugality dispatch, session 41f9b649). For grounding-quality dispatches this is a bonus; for BLIND/quarantined arms (AP6-class) it is a leak channel — packet-only isolation via claude-cli subscription transport is NOT guaranteed. Blind arms must use a transport with verified-empty system context or run as fresh headless processes, and grep outputs for quarantine-unique strings regardless.
