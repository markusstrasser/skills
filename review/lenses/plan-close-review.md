...
4. **Build a single review packet from the actual diff and touched files.**
   - Do not hand another model a prose summary of your changes if the real diff
     is available.
   - Do not split the packet across multiple ad hoc context files.
   - Use:
   ```bash
   ...
   ```
   - When the worktree is clean (everything committed — the common case for
     plan-close), pass `--since` to review the session's commit range:
   ```bash
   uv run python3 ${CLAUDE_SKILL_DIR}/scripts/build_plan_close_context.py \
     --repo "$(pwd)" \
     --since <first-session-commit> \
     --output .model-review/plan-close-context.md
   ```
   - For explicit commit-range review:
   ```bash
   uv run python3 ${CLAUDE_SKILL_DIR}/scripts/build_plan_close_context.py \
     --repo "$(pwd)" \
     --base <old-ref> \
...
For plan-close packets, the default review path is `--extract --verify`.
The shared review script emits `disposition.md`, `verified-disposition.md`, and
`coverage.json`.
...
4. Inspect `coverage.json`.
5. Summarize: what the review found, what tests were added, `validate-code` status, `validate-data` status, remaining live compatibility boundaries.
