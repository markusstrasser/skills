...
`--flex` = 50% best-effort discount (variable latency, sheds load with 503s). **llmx now AUTO-RETRIES those 503s** (backoff+jitter, `LLMX_MAX_RETRIES` default 4; llmx@0e24d5c), so a `--flex` blip no longer silently drops an item — this fixed the root cause of the 2026-06-13 corruption (a chunked extraction got 1 claim vs 22, 7/8 chunks 503-dropped, nearly inverting an eval verdict). Residual guidance: (a) for LARGE bulk jobs `llmx batch` is still cheaper + more predictable than per-call `--flex`; (b) only a SUSTAINED outage now surfaces (exit 3 after retries exhaust) — a per-call loop must still handle exit 3, not swallow it (`rc!=0 → []` is the anti-pattern); (c) add a validity guard for impossible results. Don't use `--fallback` in evals (it swaps to a weaker model mid-run, changing the arm). `--schema`, `--search`, and `--max-tokens` all work natively on the API now (no CLI-fallback gymnastics); without `--max-tokens` the API defaults to 8K output, so pass `--max-tokens 65536` for long outputs.
