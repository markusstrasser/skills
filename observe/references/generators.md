# Leverage mode: `generators` — mint new idea-generators from miss patterns

> Turn "we keep finding gold off-trail" into a new trail. A generator is anything that
> mechanically produces candidate ideas: a frame-menu entry, a checklist question, a brainstorm
> perturbation, an audit lens. This mode extracts NEW generators from the pattern of findings
> your existing generators MISSED — the meta-move that grows the divergent-discovery process
> itself. Where the core mode and `missing` mode *find* things, this mode improves *what does
> the finding*.
...
1. **Collect the out-of-band finds** (last N days' adopted findings that no standing generator
   produced). For each, one line: what was found + what question WOULD have produced it.
...
3. **Retrodiction-test** each candidate: would it have produced ≥2 of the collected finds
   ...
   the finds, not a generator. Write the retrodiction into the generator's text (it doubles as
   the usage example).
...
5. **Install one level up, with ownership.** Generators live in owner-controlled standing files
   (charter menus, CLAUDE.md checklists, skill steps) — propose to the owner if that isn't you;
   standing instructions are owned one level up from their executor.
...
7. **Retirement rule:** a generator that cycles twice with zero adopted output gets demoted to
   an appendix; menus accrete cruft exactly as fast as codebases.
...
- Installing generators in your own executor-level notes where they decay — they belong in the
  owner's standing file.
