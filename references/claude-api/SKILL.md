...
> - **Effort levels recalibrated** vs 4.7: `medium` allows somewhat more thinking, `high` somewhat less, `xhigh` substantially more. Re-baseline cost/latency at your chosen level before tuning.
> - **Mid-conversation system messages:** 4.8 accepts `role: "system"` messages *immediately after a user turn* in the `messages` array (subject to placement rules) — earlier models including 4.7 reject this with 400. Keeps prompt-cache hits on earlier turns instead of rebuilding history. Top-level `system` field is still for from-the-start instructions.
> - **Refusal `stop_details`** (present since 4.7) is now publicly documented: refusals carry a refusal *category* alongside the `refusal` stop reason. No beta header, no opt-out.
> - **Lower prompt-cache minimum:** 1,024 tokens (down from 4.7's higher floor) — short prompts that couldn't cache on 4.7 now can.
> - Materially more honest: flags uncertainty, ~4× less likely than 4.7 to let its own code flaws pass unremarked. 1M context is the default (no beta header); fast mode 2.5× speed at $10/$50.
